  ...
  if (cartItems.length === 0) {
    return (
      <div className="w-full min-h-[75vh] bg-[#F9F7F3] flex items-center justify-center p-4" style={{ fontFamily: "'Outfit', sans-serif" }}>
        ...
        >
          <div className="w-24 h-24 mx-auto bg-[#EBF5EA] text-[#7DC57A] rounded-full flex items-center justify-center mb-8 shadow-inner">
            ...
            <AnimatePresence>
              {cartItems.map((item) => (
                <motion.div 
                  ...
                  className="bg-white rounded-[2rem] p-6 shadow-xl shadow-gray-200/40 border border-gray-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 group"
                >
                  
                  {/* Product Details (Image + Info) */}
                  <div className="flex items-center gap-6 w-full sm:w-auto">
                    <div className="w-24 h-24 md:w-28 md:h-28 bg-gray-50 flex items-center justify-center overflow-hidden flex-shrink-0 rounded-2xl border border-gray-100 relative">
                      {item.product.image ? (
                        ...
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1} className="w-8 h-8 text-gray-300"><path strokeLinecap="round" strokeLinejoin="round" d="M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159m-1.5-1.5l1.409-1.409a2.25 2.25 0 013.182 0l2.909 2.909M4.5 21h15M3.75 3.75h16.5M12 3.75v13.5" /></svg>
                      )}
                    </div>

                    <div className="flex flex-col">
                      <h3 className="text-xl font-bold text-gray-900 mb-1 leading-tight" style={{ fontFamily: "'Cormorant Garamond', serif" }}>
                        {item.product.name}
                      </h3>
                      ...
                      <div className="flex items-center gap-3">
                        <span className="font-bold text-[#0F2318] bg-gray-50 border border-gray-200 px-3 py-1.5 rounded-lg text-sm shadow-sm">
                          Qty: {item.quantity}
                        </span>
                  ...
                  {/* Price & Remove Action */}
                  <div className="flex sm:flex-col items-center sm:items-end justify-between w-full sm:w-auto border-t sm:border-none border-gray-100 pt-4 sm:pt-0">
                    <p className="text-2xl font-black text-[#2D6A27] sm:mb-4">
                      KES {(item.variant.price * item.quantity).toLocaleString()}
                    </p>
                    <button 
                      onClick={() => removeFromCart(item.product.id, item.variant.id)}
                      className="text-xs font-bold text-red-400 hover:text-red-600 uppercase tracking-widest flex items-center gap-1.5 transition-colors bg-red-50 hover:bg-red-100 px-3 py-1.5 rounded-md"
                    >
          ...
          >
            <div className="bg-[#0F2318] rounded-[2.5rem] p-8 shadow-2xl relative overflow-hidden text-[#F5EDD8]">
              ...
              <motion.div whileHover={buttonHover} whileTap={buttonTap} className="mt-8">
                <Link 
                  to="/checkout"
                  className="w-full bg-[#C4892A] text-[#0F2318] py-4 rounded-xl font-bold text-lg hover:bg-[#b07b25] transition-colors shadow-[0_10px_20px_rgba(196,137,42,0.3)] flex justify-center items-center gap-3"
                >
                  Proceed to Checkout
